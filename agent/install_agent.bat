@echo off
:: ============================================================================
::  Radiology PC Tracker v1 - 1-Click Interactive Room & Desk Agent Installer
:: ============================================================================
...
echo ----------------------------------------------------------------------------
echo  BU BILGISAYAR FIZIKSEL OLARAK HANGI ODADA BULUNUYOR?
echo ----------------------------------------------------------------------------
echo  [1]  Genel PACS Oda 1 (PC 1 - 8)
...
set MASA_SEC=1
if "%ODA_SEC%"=="1" (
    set /p MASA_SEC="Genel PACS Oda 1 icindeki Masa Numarasi [1-8]: "
)
if "%ODA_SEC%"=="2" (
    set /p MASA_SEC="Genel PACS Oda 2 icindeki Masa Numarasi [1-8]: "
)
if "%ODA_SEC%"=="3" (
    set /p MASA_SEC="Genel PACS Oda 3 icindeki Masa Numarasi [1-11]: "
)
if "%ODA_SEC%"=="4" (
    set /p MASA_SEC="Genel PACS Oda 4 icindeki Masa Numarasi [1-10]: "
)
if "%ODA_SEC%"=="6" (
    set /p MASA_SEC="Toplanti Odasi icindeki Masa Numarasi [1-3]: "
)
if "%ODA_SEC%"=="7" (
    set /p MASA_SEC="KVC PACS Odasi icindeki Masa Numarasi [1-2]: "
)
if "%ODA_SEC%"=="8" (
...
powershell -ExecutionPolicy Bypass -NoProfile -Command "& {
    ...
    $agentId = ''
    $friendlyName = 'Otomatik'
    $roomName = 'Otomatik'

    if ($selected) {
        $agentId = $selected.id
        $friendlyName = $selected.name
    ...
    Set-Content -Path 'C:\ProgramData\RadTracker\config.json' -Value $cfgJson -Encoding UTF8
    ...
    try {
        ...
        Write-Host '  [BILGI] Ajan arka planda sunucuya baglanmaya devam edecek.' -ForegroundColor DarkYellow
    }
    Write-Host '============================================================' -ForegroundColor DarkCyan
}"
